...
Scope: Generic platform foundation plus completed capability integrations through Stock Movements, Accounts, Reports, Alerts, Purchases, and Supplier Payments
...
- Supplier Payments owns the nested `payments.supplier` namespace with 17 authoritative backend controls: 1 module, 2 real presentation features (`moduleInfo`, `paymentDateFilter`), 9 real fields, and 5 actions (`post`, `postInvoiceSpecific`, `inspect`, `viewLedger`, `correct`). Supplier list/post/detail, supplier ledger, unpaid-purchase lookup, and reconciliation endpoints enforce the module and relevant action after subscription/RBAC checks. Optional notes and invoice-specific posting are enforced in the service; supplier correction is enforced after the shared correction workflow resolves the payment party. Generic scoped reset removes only `payments.supplier.*` overrides.
...
- Supplier Payments frontend TypeScript project references typecheck: passed.
