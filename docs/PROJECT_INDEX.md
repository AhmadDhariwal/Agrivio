...
* F06 Phase 1 complete (`R1-F06-001`–`R1-F06-003`) — see [tasks/F06-PHASE-1-SALES-FOUNDATION.md](tasks/F06-PHASE-1-SALES-FOUNDATION.md); customer payment/ledger foundation, sale drafts, invoice numbering, and session-safe deduplicated Customer selector caching
...
* Customer accounting audit corrections complete (2026-09-20): automatic advance consumption/reversal, advance-aware credit exposure, opening-receivable FIFO allocation, immutable opening correction, and dashboard receivable/advance/net-exposure KPIs — see [tasks/F06-PHASE-2-SALE-POSTING.md](tasks/F06-PHASE-2-SALE-POSTING.md)
...
* Treasury / Account Management Phase 1 backend hardening complete (2026-09-23): delta-only set-balance adjustment with stale-state protection, categorized manual treasury flows, account-level movement serialization, filtered history, bulk balances, active liquid-funds summary, and append-only reversal — see [tasks/F07-PHASE-3-ACCOUNT-TRANSACTIONS-EXPENSES.md](tasks/F07-PHASE-3-ACCOUNT-TRANSACTIONS-EXPENSES.md)
* Customer Financial Phase 2 backend complete (2026-09-24): principal-only customer loans, targeted repayments/reversals, separate loan receivable, target-aware trade/advance/loan balance adjustments, stale-state protection, derived exposure, and dashboard aggregation — see [tasks/CUSTOMER-FINANCIAL-PHASE-2.md](tasks/CUSTOMER-FINANCIAL-PHASE-2.md)
* Supplier Financial Phase 3 backend complete (2026-09-24): supplier advance refunds/reversals, target-aware payable and advance adjustments/reversals, supplier financial serialization, target/ledger reconciliation protection, and frontend API handoff — see [tasks/SUPPLIER-FINANCIAL-PHASE-3.md](tasks/SUPPLIER-FINANCIAL-PHASE-3.md)
* Financial Position / Cash-Bank Phase 4 backend complete (2026-09-24): read-only financial position, daily cash position, account/cash/bank books, transfers, treasury movements, manual adjustments, loan/refund reports, reconciliation diagnostics, exports, and dashboard liquid totals — see [tasks/FINANCIAL-POSITION-CASH-BANK-PHASE-4.md](tasks/FINANCIAL-POSITION-CASH-BANK-PHASE-4.md)
...
* Staging same-origin API proxy complete (2026-09-08) — Cloudflare Pages `/api/*` Function transport avoids third-party session-cookie blocking while retaining Render session/CSRF/origin controls; see [tasks/F02-PHASE-2-SESSION-AUTHENTICATION.md](tasks/F02-PHASE-2-SESSION-AUTHENTICATION.md)
* Organization/data-operations API cache hardening complete (2026-08-30) — organization/settings/setup, branches/warehouses, employees/access, billing, audit, and imports now use scoped exact-query caching and success-only targeted invalidation; complete location option endpoints replace capped selector preloads
...
* Customer and Supplier payment correction review hardening complete (2026-09-26): transaction-visible customer replacement targets, shared invoice-specific validation, reload-derived lineage, retry-safe UI idempotency, cached active-account selection, precise invalidation, and isolated real-Mongo coverage — see [tasks/F09-PHASE-2-SECURITY-PERF-A11Y.md](tasks/F09-PHASE-2-SECURITY-PERF-A11Y.md).
* Supplier payable/advance accounting corrected (2026-09-22): opening payable participates in payment FIFO, purchases consume supplier advance with immutable paired effects, cancellation restores consumed advance, and supplier net payable/reconciliation/dashboard reporting are ledger-derived; see [tasks/F05-PHASE-3-SUPPLIER-PAYMENTS-CANCEL-RETURNS.md](tasks/F05-PHASE-3-SUPPLIER-PAYMENTS-CANCEL-RETURNS.md).
...
| [tasks/ORGANIZATION-CAPABILITY-PHASE-1.md](tasks/ORGANIZATION-CAPABILITY-PHASE-1.md) | Organization Capability & UI Policy foundation and completed module integrations through Supplier Ledger, plus Sales backend capability integration |
...
| [tasks/CUSTOMER-FINANCIAL-PHASE-2.md](tasks/CUSTOMER-FINANCIAL-PHASE-2.md) | Customer loans, loan repayments/reversals, target-aware customer balance adjustments, and derived exposure |
| [tasks/SUPPLIER-FINANCIAL-PHASE-3.md](tasks/SUPPLIER-FINANCIAL-PHASE-3.md) | Supplier advance refunds/reversals and target-aware payable/advance adjustments |
| [tasks/FINANCIAL-POSITION-CASH-BANK-PHASE-4.md](tasks/FINANCIAL-POSITION-CASH-BANK-PHASE-4.md) | Financial position, cash/bank/treasury reports, manual adjustments, and reconciliation diagnostics |
