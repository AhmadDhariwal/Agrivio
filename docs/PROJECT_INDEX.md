...
* Owner activation-token Mongo session consistency hardened (2026-09-08): activation-token reads now remain inside the active Mongoose transaction, with real replica-set coverage for approve, reissue, commit visibility, activation, and one-time reuse rejection; see [tasks/F02-PHASE-1-ORGANIZATION-ONBOARDING.md](tasks/F02-PHASE-1-ORGANIZATION-ONBOARDING.md).
