...
export class EmployeeFormPage {
  ...
  readonly canManage = computed(() =>
    this.employeeId() === null ? this.canCreate() : this.canUpdate(),
  );
      ...
      this.api
        ...
        .pipe(
          ...
          switchMap((employee) =>
            ...
            }).pipe(map((locations) => ({ employee, locations }))),
  ...
  private buildCreatePayload(value: ReturnType<typeof this.form.getRawValue>): {
    ...
    const payload = {} as { email: string; displayName: string; role: OrganizationRole };
    if (this.showEmail()) {
