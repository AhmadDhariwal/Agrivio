...
function createInMemoryCustomerFinanceStore() {
  ...
  return {
    ...
    async listLoans(org, filter, pagination) { let rows = [...loans.values()].filter((r) => String(r.organizationId) === String(org)); if (filter.customerId) rows = rows.filter((r) => String(r.customerId) === String(filter.customerId)); if (filter.fromDate) rows = rows.filter((r) => r.businessDate >= filter.fromDate); if (filter.toDate) rows = rows.filter((r) => r.businessDate <= filter.toDate); if (filter.search) rows = rows.filter((r) => `${r.reference ?? ''} ${r.notes ?? ''}`.toLowerCase().includes(filter.search.toLowerCase())); rows.sort((a,b) => String(b.businessDate).localeCompare(String(a.businessDate))); return { total: rows.length, items: rows.slice(pagination.skip, pagination.skip + pagination.pageSize).map((r) => ({ ...r })) }; },
