...
const BRANCHES_MODULE_KEY = 'branches';
...
const definitions = [
  ...
  {
    key: BRANCHES_MODULE_KEY,
    parentKey: null,
    moduleKey: BRANCHES_MODULE_KEY,
    type: CONTROL_TYPES.Module,
    label: 'Branches',
    description: 'Branch management screens and Branch API operations for this organization.',
    defaultPolicy: { enabled: true },
    configurable: { enabled: true },
    risk: RISK_LEVELS.Critical,
    requiredPermissions: { enabled: 'branches.view' },
    reason:
      'Disabling access hides Branches and blocks direct Branch API operations without deleting locations or altering invoice history.',
  },
  ...[
    ['moduleInfo', 'About Branches', 'Show the Branches guidance panel.'],
    ['search', 'Search', 'Search Branches by name, invoice prefix, or code.'],
    ['statusFilter', 'Status Filter', 'Filter Branches by active or inactive status.'],
  ].map(([id, label, description]) => ({
    key: `branches.features.${id}`,
    parentKey: BRANCHES_MODULE_KEY,
    moduleKey: BRANCHES_MODULE_KEY,
    type: CONTROL_TYPES.Feature,
    label,
    description,
    ...
    risk: RISK_LEVELS.Normal,
    requiredPermissions: { enabled: 'branches.view' },
    reason: 'This is a presentation control and does not alter Branch domain behavior.',
  })),
  {
    key: 'branches.fields.name',
    parentKey: BRANCHES_MODULE_KEY,
    moduleKey: BRANCHES_MODULE_KEY,
    type: CONTROL_TYPES.Field,
    label: 'Branch name',
    description: 'Required Branch identity field.',
    defaultPolicy: { visible: true, editable: true },
    configurable: { visible: false, editable: false },
    risk: RISK_LEVELS.Critical,
    platformEnforced: true,
    requiredPermissions: { visible: 'branches.view', editable: 'branches.manage' },
    reason: 'Branch name is required for creation and remains platform enforced.',
  },
  {
    key: 'branches.fields.invoicePrefix',
    parentKey: BRANCHES_MODULE_KEY,
    moduleKey: BRANCHES_MODULE_KEY,
    type: CONTROL_TYPES.Field,
    label: 'Invoice prefix',
    description: 'Required branch-wise invoice numbering prefix.',
    defaultPolicy: { visible: true, editable: true },
    configurable: { visible: false, editable: false },
    risk: RISK_LEVELS.Critical,
    platformEnforced: true,
    requiredPermissions: { visible: 'branches.view', editable: 'branches.manage' },
    reason: 'Invoice prefix is required for creation and remains platform enforced.',
  },
  {
    key: 'branches.fields.code',
    parentKey: BRANCHES_MODULE_KEY,
    moduleKey: BRANCHES_MODULE_KEY,
    type: CONTROL_TYPES.Field,
    label: 'Branch code',
    description: 'Optional short Branch reference code.',
    defaultPolicy: { visible: true, editable: true },
    configurable: { visible: true, editable: true },
    risk: RISK_LEVELS.Normal,
    requiredPermissions: { visible: 'branches.view', editable: 'branches.manage' },
  ...
  {
    key: 'branches.fields.status',
    parentKey: BRANCHES_MODULE_KEY,
    moduleKey: BRANCHES_MODULE_KEY,
    type: CONTROL_TYPES.Field,
    label: 'Lifecycle status',
    description: 'Active or inactive Branch state.',
    defaultPolicy: { visible: true, editable: true },
    configurable: { visible: true, editable: true },
    risk: RISK_LEVELS.Recommended,
    requiredPermissions: { visible: 'branches.view', editable: 'branches.manage' },
    reason: 'Status changes remain subject to dedicated lifecycle actions and existing validation.',
  },
  ...[
    ['create', 'Create branch', 'branches.manage', RISK_LEVELS.Recommended],
    ['edit', 'Edit branch', 'branches.manage', RISK_LEVELS.Recommended],
    ['deactivate', 'Deactivate branch', 'branches.manage', RISK_LEVELS.Recommended],
    ['reactivate', 'Reactivate branch', 'branches.manage', RISK_LEVELS.Recommended],
    ['delete', 'Delete permanently', 'branches.manage', RISK_LEVELS.Critical],
    ['refresh', 'Refresh', 'branches.view', RISK_LEVELS.Normal],
  ].map(([id, label, permission, risk]) => ({
    key: `branches.actions.${id}`,
    parentKey: BRANCHES_MODULE_KEY,
    moduleKey: BRANCHES_MODULE_KEY,
    type: CONTROL_TYPES.Action,
    label,
    ...
    ...(id === 'delete'
      ? {
          reason:
            'The policy can block deletion but cannot bypass record-in-use protection.',
        }
      : {}),
  })),
  {
  ...
  ...[
    ...
    ['financial-position', 'Financial Position Report'],
    ['daily-cash-position', 'Daily Cash Position Report'],
    ['account-statement', 'Account Statement Report'],
    ['cash-book', 'Cash Book Report'],
    ['bank-book', 'Bank Book Report'],
    ['account-transfers', 'Account Transfers Report'],
    ['treasury-movements', 'Treasury Movements Report'],
    ['manual-adjustments', 'Manual Financial Adjustments Report'],
    ['customer-loans', 'Customer Loans Report'],
    ['supplier-refunds', 'Supplier Refunds Report'],
    ['financial-reconciliation', 'Financial Reconciliation Report'],
